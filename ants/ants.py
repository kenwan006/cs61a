...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if self.exit:  #if the exit in not None place, this place must have an entrance to reach 
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    insect.ant, self.ant = self.ant, insect #ant in the place is insect, ant in insect is the old ant
                else:   
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return self.place.ant is not None and self.place.ant.blocks_path # only there is ant and ant blocks the path, return True..
...
class Ant(Insect):
    ...
    food_cost = 0
    blocks_path = True
    container = False  # only the BodyguardAnt.container is true
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        return self.container and not self.ant and not other.container #the ant self is container; not contain any ant yet; the ant ready to be contained is not container
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        steps = 0
        place = self.place  #don't assign like self.place = self.place.entrance. it would change the place attribute of the objecct (self)
        bees = place.bees
        while not bees and place.entrance != hive:  #if the bees is empty, then move forward to the next place
            place = place.entrance
            bees = place.bees
            steps += 1

        if self.min_range <= steps <= self.max_range:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3
        "*** YOUR CODE HERE ***"
        Place.add_insect(self, insect) #add the insect whether it's watersafe or not

        if not insect.watersafe:  #if it's not water safe, reduce it's armor to 0 and remove the insect
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:                     #if the FireAnt's armor reaches 0
            bees_temp = []
            for bee in self.place.bees:
                bee.reduce_armor(self.damage)  # each bee will get its armor reduced by the damage
                if bee.armor:                  # after reduction, if the armor is not 0, then add the bee to the list of bees
                    bees_temp.append(bee)
            self.place.bees = bees_temp
            self.place.ant = None
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        if self.digesting:              #if the digesting number is not 0, then need decrement
            self.digesting -= 1
        else:    #digesting is 0, start eeating..
            bee = random_or_none(self.place.bees)      #choose a bee from the place in random
            if bee: # if the bee is not none
                self.eat_bee(bee)
                self.digesting = self.time_to_digest   #after eating a bee, reset the digesting number
        # END Problem 10
...
# END Problem 13
    ...
    armor = 1
    firstqueen = True
    doubled_ants = [] # ants with already doubled damages..
    # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.firstqueen:
            ...
            while next_place:     #continue to move forward until next place is None
                if next_place.ant:
                    if next_place.exit:
                        if next_place.ant.container: #ant is an container
                            if next_place.ant.ant and not next_place.ant.ant in self.doubled_ants:
                                next_place.ant.ant.damage *= 2
                                self.doubled_ants.append(next_place.ant.ant)
                        if next_place.ant not in self.doubled_ants:
                            next_place.ant.damage *= 2
                            self.doubled_ants.append(next_place.ant)
                    else:
                        bees_win() #reaches to the end of the tunel, the game is over..
