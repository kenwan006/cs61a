...
class VendingMachine:
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            self.balance = 0 # once finish purchase, reset the balance to 0
            return 'Here is your {0} and ${1} change.'.format(self.food, amount)
          

    def deposit(self, amount):
        self.balance = self.balance + amount
        if self.stock == 0:  #if no stock, no matter how much to deposit, it won't work
            self.balance = 0
            return 'Machine is out of stock. Here is your ${0}.'.format(self.balance + amount)    
        else:
...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        "*** YOUR CODE HERE ***"
        message = message[len('please '):] # remove the 'please'
